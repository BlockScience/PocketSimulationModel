...
name_map = lambda x: "_".join(f"{replacement_dict.get(key, key)}:{val}" for key, val in x.items())

def build_individual_dict_from_param_grid(param_grid: Dict[str, List],
                                          name_map_to_use: Callable[str, str] = name_map):
...
def get_data_from_param_grid(param_grid: Dict[str, List],
...
def mock_get_data_from_param_grid(param_grid: Dict[str, List],
                             base_params_str = "Base",
                             other_params_to_sweep: Dict[str,List] = None,
                             prefix: str = "individual"):
...
def give_data_individual_names(df: pd.DataFrame,
                               col_name: str = "individual_name",
                               replacement_dict_to_use: Dict[str,str] = None) -> pd.DataFrame:
    print("Finding names to use for individual.")
    if replacement_dict_to_use is None:
        replacement_dict_to_use = replacement_dict
    
    assert not(replacement_dict_to_use is None), "The replacement dict should be set by now."

    binary_param_cols = [col 
    ...
    df[col_name] = df.apply(lambda row: '_'.join([f"{replacement_dict_to_use[col]}:{val}" 
                                                                  for col, val 
                                                                  in row[binary_param_cols].items()]), 
                                                                  axis=1)
...
def find_best_individual_name(df: pd.DataFrame,
    ...
    best_individuals = [key for key in fitness_dict.keys() if 
                        isclose(fitness_dict[key], max_val)]
...
def update_param_grid(old_param_grid: Dict[str, List],
                        best_individual: Dict[str, Union[float,int]],
                        relevant_keys: set = None,) -> Dict[str,List]:
    """
    ...
    assert all([len(val) == 2 for key,val in old_param_grid.items()
    ...
    for key in old_param_grid.keys():
        if key in relevant_keys:
            ...
            if isclose(best_val,lb):
                ...
                raise Exception("Something is wrong. The best individual does not have its values close to either the prior upper or lower bound.")
        else:
            new_param_grid[key] = old_param_grid.get(key)

    return new_param_grid

def iterate_adaptive_multigrid(initial_param_grid: Dict[str, List],
    ...
    while(not(finished)):
        ...
        data = get_data_from_param_grid(param_grid = new_param_grid,
                                        other_params_to_sweep = other_params_to_sweep,
                                        prefix = prefix)
        data = give_data_individual_names(data) 

        new_best_name, new_best_val = find_best_individual_name(data,
                                                                fitness_func = fitness_func,
                                                                name_biased_towards = old_best_name)
        
        assert not(new_best_name is None), "The best individual should be somewhere in this dictionary."
        print(new_best_name)
        ...
        if not(finished):
            new_param_grid = update_param_grid(old_param_grid = old_param_grid,
