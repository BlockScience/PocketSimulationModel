...
application_config = {"Test": [Application(name = "A1", pokt_holdings = 1000, staked_pokt = 1000,
           services = ["S1", "S2"], geo_zone = "G1", number_of_services=1,
           stake_status = "Staked", unstaking_height = None, delegate = None), Application(name = "A2", pokt_holdings = 5000, staked_pokt = 1000,
           services = ["S2", "S3"], geo_zone = "G2", number_of_services=2,
           stake_status = "Staked", unstaking_height = None, delegate = None), Application(name = "A3", pokt_holdings = 3000, staked_pokt = 1000,
...
portals_config = {"Test": [Portal(name = "P1", stake_status = "Staked", delegators = ["A3"],
                          pokt_holdings = 100, staked_pokt = 100)]}

service_config = {"Test": [Service(name = "S1", portal_api_prefix = "S1", service_id = "S1"),
...
for i in range(1,6):
    servicers_config["Test"].append(Servicer(name = "X{}".format(i), servicer_salary = 0, report_card = None,
                     test_scores = None, pokt_holdings = 1000, staked_pokt= 1000,
                     service_url = None, services = [], geo_zone = "G{}".format(i%3+1),
                     operator_public_key = None, pause_height = None, stake_status = "Staked",
                     unkstaking_height = None))
    
validators_config = {"Test": [Validator(name = "Mega Validator", pokt_holdings = 1000,
                 staked_pokt = 1000, service_url = None,
                 operator_public_key = None, stake_status = "Staked")]}