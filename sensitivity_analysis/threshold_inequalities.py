...
THRESHOLD_INEQUALITIES = {
    ...
    'circulating_supply_available_supply_ratio'  : lambda df, min, max       : threshold_kpi_ratios(df, min, max, 'circulating_supply+available_supply'),
    ...
    'net_inflation_dao_value_capture_elasticity' : lambda df, min, max       : threshold_elasticity(df, min, max, 'net_inflation+dao_value_capture'),
...
def threshold_average(df, min, max, entity):
    ...
    if entity not in ['servicer_capital_costs', 'net_inflation']:
        raise ValueError("Error: unsupported threshold inequality type")
...
def threshold_kpi_ratios(df, min, max, entity):
    ...
    if entity not in ['circulating_supply+available_supply']:
        raise ValueError("Error: unsupported threshold inequality type")
...
def threshold_elasticity(df, min, max, entity):
    ...
    if entity not in ['net_inflation+dao_value_capture']:
        raise ValueError("Error: unsupported threshold inequality type")
